...
@pytest.mark.asyncio
async def test_authoritative_assignment_creation_and_canonical_events(client):
    ...
    with patch.object(sio, "emit", new_callable=AsyncMock) as mock_sio_emit:
        ...
        assign_created_calls = [
            call for call in mock_sio_emit.call_args_list if call.args[0] == "assignment.created"
        ]
...
@pytest.mark.asyncio
async def test_full_state_flow_progression(client):
...
@pytest.mark.asyncio
async def test_responder_location_updated_canonical_event(client):
...
@pytest.mark.asyncio
async def test_citizen_tracking_reconnection_and_persistence(client):
